...
public class TemplatePass : ScriptableRenderPass
{
	...
	public TemplatePass(TemplateFeature.PassSettings passSettings)
	{
		...
		if (material == null) material = CoreUtils.CreateEngineMaterial("Hidden/Blur");
	...
	{
		//Get the volume component from the current stack
		TemplateVolumeComponent component = VolumeManager.instance.stack.GetComponent<TemplateVolumeComponent>();
		if (component.IsActive())
		{
			//If the component is active, set the appropriate variables and render
			material.SetFloat(BlurStrengthProperty, component.intensity.value);
